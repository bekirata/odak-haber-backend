                ...
                for token_doc in tokens:
                    ...
                    if push_token:
                        result = await send_expo_push_notification(
                            push_token,
                            "📋 Günün Özeti Hazır!",
                            f"Bugünün en önemli {len(summary_items)} haberi özetlendi. Hemen okuyun!",
                            {"type": "daily_summary"}
                        )
...
async def text_to_speech(request: Request):
    ...
    try:
        ...
        ai_client = AsyncOpenAI(api_key=EMERGENT_LLM_KEY, base_url="https://integrations.emergentagent.com/llm")
        speech_response = await ai_client.audio.speech.create(model="tts-1-hd", input=text, voice="onyx", speed=0.95, response_format="mp3")
