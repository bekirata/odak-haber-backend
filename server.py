...
@api_router.get("/news")
async def get_cached_news(category: str = None, limit: int = 100, feed_urls: str = None, since_iso: str = None):
    """Get news from cache.

    Query params:
      - category: filter by category (or 'TÜMÜ' for all)
      - limit: max items to return (default 100, hard cap 500 for performance)
    ...
    """
    # Hard cap to protect backend & network
    if limit > 500:
        limit = 500
    if limit < 1:
        limit = 100
    ...
    # Projection: exclude only large/unused fields for faster transfer
    projection = {"_id": 0}

    news = await db.news_cache.find(query, projection).sort("pub_date_iso", -1).limit(limit).to_list(limit)
